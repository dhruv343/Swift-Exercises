...
struct RunningWorkout {
    ...
    func printout(){
        print("The average mile time is \(averageMileTime) for distance \(distance) and time \(time)")
    }
}
var user1=RunningWorkout(distance: 32000, time: 5, elevation: 34);
user1.printout();
...
struct Steps {
    var steps: Int{
        willSet{
            if(newValue==goal){
                print("Congrats you have reached the goal")
            }
        }
    }
    var goal: Int
...
var steps=Steps(steps: 999, goal: 1000)
steps.takeStep()
